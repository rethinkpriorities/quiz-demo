...
function datasetManifestPlugin() {
  function generate() {
    ...
    const manifest = files.map(filename => {
      ...
      return { id, filename, name: data.name || id };
    });
  ...
  return {
    ...
    configureServer(server) {
      generate();
      server.watcher.on('add', f => { if (f.includes('config/datasets')) generate(); });
      server.watcher.on('unlink', f => { if (f.includes('config/datasets')) generate(); });
    },
  };
}
